...
static int dihos_raw_to_friendly(const char *raw, char *out, uint32_t cap)
{
    if (!raw || !out || cap < 2u)
        return -1;
    ...
    out[0] = '/';
    dihos_copy_trunc(out + 1, cap - 1u, raw + 3);
    return 0;
}

static int dihos_canonicalize_friendly(const char *source, char *out, uint32_t cap)
{
    uint32_t restore[32];
...
static int dihos_resolve_friendly_path(const char *input, char *out)
{
    char joined[DIHOS_SHELL_PATH_CAP * 2u];

    if (!input || !input[0])
        return dihos_copy_trunc(out, DIHOS_SHELL_PATH_CAP, G_shell.cwd), 0;

    if (strncmp(input, "0:/", 3u) == 0)
    {
        if (dihos_raw_to_friendly(input, joined, sizeof(joined)) != 0)
            return -1;
        return dihos_canonicalize_friendly(joined, out, DIHOS_SHELL_PATH_CAP);
    }

    if (input[0] == '/')
        return dihos_canonicalize_friendly(input, out, DIHOS_SHELL_PATH_CAP);

    joined[0] = 0;
    dihos_copy_trunc(joined, sizeof(joined), G_shell.cwd);

    if (strcmp(joined, "/") != 0 && dihos_append_cstr(joined, sizeof(joined), "/") != 0)
        return -1;

    if (dihos_append_cstr(joined, sizeof(joined), input) != 0)
        return -1;

    return dihos_canonicalize_friendly(joined, out, DIHOS_SHELL_PATH_CAP);
}

static int dihos_friendly_to_raw(const char *friendly, char *raw, uint32_t cap)
{
    ...
    {
        dihos_copy_trunc(raw, cap, "0:/");
        return 0;
    }

    if (strlen(friendly) + 3u >= cap)
        return -1;

    raw[0] = '0';
    raw[1] = ':';
    dihos_copy_trunc(raw + 2, cap - 2u, friendly);
    return 0;
}

static int dihos_resolve_raw_path(const char *input, char *friendly, char *raw)
{
    if (dihos_resolve_friendly_path(input, friendly) != 0)
        return -1;
    return dihos_friendly_to_raw(friendly, raw, DIHOS_SHELL_PATH_CAP);
}
...
static int dihos_join_raw_path(const char *base, const char *name, char *out, uint32_t cap)
{
    ...
    if (!base || !name || !out || !cap)
        return -1;

    base_len = (uint32_t)strlen(base);
    if (base_len + strlen(name) + 2u >= cap)
        return -1;

    dihos_copy_trunc(out, cap, base);
    if (strcmp(base, "0:/") != 0 && dihos_append_cstr(out, cap, "/") != 0)
        return -1;
    if (dihos_append_cstr(out, cap, name) != 0)
        return -1;
    return 0;
}

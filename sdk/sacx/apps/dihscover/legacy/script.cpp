...
int browser_scripts_start(browser_document*doc,uint64_t now)
{
    ...
    for(uint32_t i=1u;i<doc->node_count&&scripts<DIHSCOVER_SCRIPT_CAP&&total<131072u;++i)if(doc->nodes[i].tag==B_TAG_SCRIPT&&doc->nodes[i].text_off){const char*s=browser_document_string(doc,doc->nodes[i].text_off);uint32_t n=b_strlen(s);if(n>32768u||total+n>131072u||(!find_text(s,"getElementById")&&!find_text(s,"querySelector")))continue;total+=n;++scripts;register_click_handlers(s);schedule_timers(s,now);if(!find_text(s,"setTimeout")&&!find_text(s,"addEventListener"))changed+=execute_dom_updates(s);}
    return changed;
}

...
enum { LOAD_IDLE, LOAD_DOCUMENT, LOAD_STYLES, LOAD_SCRIPTS, LOAD_IMAGES };
#define DIHSCOVER_STYLESHEET_CAP 32u
#define DIHSCOVER_EXTERNAL_SCRIPT_CAP 8u
#define DIHSCOVER_EXTERNAL_SCRIPT_BYTES 32768u
enum {
    UI_TITLEBAR_H = 34,
    UI_TOOLBAR_Y = 42,
    UI_STATUS_Y = 84,
...
typedef struct browser_external_script { uint8_t used, loaded, failed, reserved; uint32_t node_index; char url[DIHSCOVER_URL_CAP]; } browser_external_script;
...
static browser_stylesheet g_styles[DIHSCOVER_STYLESHEET_CAP];
static browser_external_script g_external_scripts[DIHSCOVER_EXTERNAL_SCRIPT_CAP];
...
{
    uint32_t count=0u;int decoded_inline=0;
    ...
    for(uint32_t i=1u;i<g_doc.node_count&&count<DIHSCOVER_IMAGE_CAP;++i)if(g_doc.nodes[i].tag==B_TAG_SVG&&g_doc.nodes[i].text_off&&g_doc.nodes[i].text_len){
        browser_node*n=&g_doc.nodes[i];browser_image*im=&g_images[count];im->used=1u;im->node_index=i;im->generation=g_generation;n->image_slot=(int16_t)count;
        if(g_api->img_load_memory(browser_document_string(&g_doc,n->text_off),n->text_len,&im->image)==0){uint32_t iw=0u,ih=0u;if(g_api->img_size&&g_api->img_size(im->image,&iw,&ih)==0&&iw&&ih&&!n->attr_width&&!n->attr_height&&n->style.width_px==120u&&n->style.height_px==40u){uint32_t maxw=g_view_w>48u?g_view_w-48u:g_view_w;uint32_t w=iw>maxw?maxw:iw;n->style.width_px=(uint16_t)(w?w:1u);n->style.height_px=(uint16_t)(((uint64_t)ih*n->style.width_px)/iw);if(!n->style.height_px)n->style.height_px=1u;}decoded_inline=1;}else im->failed=1u;
        ++count;
    }
...
static void collect_stylesheets(void)
{
    ...
    for(uint32_t i=1u;i<g_doc.node_count&&count<DIHSCOVER_STYLESHEET_CAP;++i){browser_node*n=&g_doc.nodes[i];if(n->tag!=B_TAG_LINK||!n->href_off||!n->rel_off)continue;const char*rel=browser_document_string(&g_doc,n->rel_off);uint32_t p=0u;int sheet=0;while(rel[p]){while(rel[p]==' '||rel[p]=='\t')++p;uint32_t a=p;while(rel[p]&&rel[p]!=' '&&rel[p]!='\t')++p;if(p-a==10u){sheet=1;for(uint32_t k=0;k<10u;++k){char c=rel[a+k];if(c>='A'&&c<='Z')c=(char)(c+32);if(c!="stylesheet"[k]){sheet=0;break;}}}if(sheet)break;}if(!sheet)continue;const char*href=browser_document_string(&g_doc,n->href_off);if(contains_text(g_doc.base_url,"html.duckduckgo.com")&&contains_text(href,"duckduckgo.com"))continue;browser_stylesheet*s=&g_styles[count++];s->used=1u;if(browser_url_resolve(g_doc.base_url,href,s->url,sizeof(s->url))!=0)s->failed=1u;}
}

static int start_next_stylesheet(void)
{
    for(uint32_t i=0u;i<DIHSCOVER_STYLESHEET_CAP;++i)if(g_styles[i].used&&!g_styles[i].loaded&&!g_styles[i].failed){sacx_net_request_desc d;b_memset(&d,0,sizeof(d));d.url=g_styles[i].url;d.max_response_bytes=512u*1024u;d.timeout_ms=30000u;d.redirect_limit=5u;if(g_api->net_request_start(&d,&g_request)==0){g_load_state=LOAD_STYLES;set_status("Loading stylesheet");return 1;}g_styles[i].failed=1u;}return 0;
}

static void collect_external_scripts(void)
{
    b_memset(g_external_scripts,0,sizeof(g_external_scripts));uint32_t count=0u;
    for(uint32_t i=1u;i<g_doc.node_count&&count<DIHSCOVER_EXTERNAL_SCRIPT_CAP;++i){browser_node*n=&g_doc.nodes[i];if(n->tag!=B_TAG_SCRIPT||!n->src_off||n->text_off)continue;if(n->type_off){const char*type=browser_document_string(&g_doc,n->type_off);if(contains_text(type,"module")||contains_text(type,"json"))continue;}browser_external_script*s=&g_external_scripts[count++];s->used=1u;s->node_index=i;if(browser_url_resolve(g_doc.base_url,browser_document_string(&g_doc,n->src_off),s->url,sizeof(s->url))!=0)s->failed=1u;}
}

static int start_next_external_script(void)
{
    for(uint32_t i=0u;i<DIHSCOVER_EXTERNAL_SCRIPT_CAP;++i)if(g_external_scripts[i].used&&!g_external_scripts[i].loaded&&!g_external_scripts[i].failed){sacx_net_request_desc d;b_memset(&d,0,sizeof(d));d.url=g_external_scripts[i].url;d.max_response_bytes=DIHSCOVER_EXTERNAL_SCRIPT_BYTES;d.timeout_ms=20000u;d.redirect_limit=5u;if(g_api->net_request_start(&d,&g_request)==0){g_load_state=LOAD_SCRIPTS;set_status("Loading script");return 1;}g_external_scripts[i].failed=1u;}return 0;
}

static void finalize_document(void)
{
    if(g_api->log)g_api->log("Dihscover: layout begin");browser_document_apply_site_defaults(&g_doc);browser_document_layout(&g_doc,g_view_w);if(browser_document_make_readable(&g_doc)){if(g_api->log)g_api->log("Dihscover: readable fallback");browser_document_layout(&g_doc,g_view_w);}if(g_api->log)g_api->log("Dihscover: layout done");b_copy(g_history[g_history_at].url,sizeof(g_history[g_history_at].url),g_doc.url);(void)g_api->textbox_set_text(g_textbox,g_doc.url);(void)g_api->window_set_title(g_window,g_doc.title);(void)g_api->gfx_text_set(g_tls_obj,g_document_tls_unverified?"TLS: provisional":"");clear_images();if(collect_images())browser_document_layout(&g_doc,g_view_w);if(g_api->log)g_api->log("Dihscover: scripts begin");(void)browser_scripts_start(&g_doc,g_api->time_ticks());if(g_api->log)g_api->log("Dihscover: document ready");g_load_state=LOAD_IMAGES;set_status(g_doc.truncated?"Loaded safely (truncated)":"Loaded");
}
...
static void finish_document(void)
{
    ...
    g_document_tls_unverified=info.tls_unverified;collect_stylesheets();collect_external_scripts();if(!start_next_stylesheet()&&!start_next_external_script())finalize_document();
}
...
{
    if(!g_request){if(!start_next_stylesheet()&&!start_next_external_script())finalize_document();return;}uint32_t st=g_api->net_request_status(g_request);if(st==SACX_NET_STATUS_QUEUED||st==SACX_NET_STATUS_LOADING)return;
    browser_stylesheet*active=0;for(uint32_t i=0;i<DIHSCOVER_STYLESHEET_CAP;++i)if(g_styles[i].used&&!g_styles[i].loaded&&!g_styles[i].failed){active=&g_styles[i];break;}
    if(st==SACX_NET_STATUS_DONE&&active){sacx_net_response_info info;uint32_t got=0u;b_memset(&info,0,sizeof(info));if(g_api->net_response_info(g_request,&info)==0&&info.body_size<DIHSCOVER_BODY_CAP&&g_api->net_response_read(g_request,0u,g_body,info.body_size,&got)==0&&got==info.body_size){g_body[got]=0;(void)browser_document_apply_stylesheet(&g_doc,(const char*)g_body,got);active->loaded=1u;}else active->failed=1u;}else if(active)active->failed=1u;(void)g_api->net_request_release(g_request);g_request=0u;if(!start_next_stylesheet()&&!start_next_external_script())finalize_document();
}

static void pump_external_scripts(void)
{
    if(!g_request){if(!start_next_external_script())finalize_document();return;}uint32_t st=g_api->net_request_status(g_request);if(st==SACX_NET_STATUS_QUEUED||st==SACX_NET_STATUS_LOADING)return;browser_external_script*active=0;for(uint32_t i=0u;i<DIHSCOVER_EXTERNAL_SCRIPT_CAP;++i)if(g_external_scripts[i].used&&!g_external_scripts[i].loaded&&!g_external_scripts[i].failed){active=&g_external_scripts[i];break;}
    if(st==SACX_NET_STATUS_DONE&&active){sacx_net_response_info info;uint32_t got=0u;b_memset(&info,0,sizeof(info));if(g_api->net_response_info(g_request,&info)==0&&!info.truncated&&info.body_size<DIHSCOVER_EXTERNAL_SCRIPT_BYTES&&g_api->net_response_read(g_request,0u,g_body,info.body_size,&got)==0&&got==info.body_size&&browser_document_set_script_source(&g_doc,active->node_index,(const char*)g_body,got)==0)active->loaded=1u;else active->failed=1u;}else if(active)active->failed=1u;(void)g_api->net_request_release(g_request);g_request=0u;if(!start_next_external_script())finalize_document();
}
...
{
    if(!object)return;uint32_t width=g_api->text_measure_line_px?g_api->text_measure_line_px(line,scale,0):n->w;if(width>n->w)width=n->w;int32_t x=text_x;if(n->style.text_align==1u)x-=(int32_t)(width/2u);else if(n->style.text_align==2u)x-=(int32_t)width;(void)g_api->gfx_obj_set_rect(object,x,y,width,n->style.font_px>=24u?2u:1u);(void)g_api->gfx_obj_set_fill_rgb(object,n->style.color.r,n->style.color.g,n->style.color.b);(void)g_api->gfx_obj_set_z(object,4+n->style.z_index);(void)g_api->gfx_obj_set_visible(object,n->style.underline&&line[0]?1u:0u);
}

static int node_descends_from_main(uint32_t node,uint32_t ancestor){for(int32_t p=(int32_t)node;p>=0;p=g_doc.nodes[p].parent)if((uint32_t)p==ancestor)return 1;return 0;}
static int list_item_prefix(uint32_t index,char*out,uint32_t cap)
{
    int32_t li=-1;for(int32_t p=g_doc.nodes[index].parent;p>0;p=g_doc.nodes[p].parent)if(g_doc.nodes[p].tag==B_TAG_LI){li=p;break;}if(li<0)return 0;for(uint32_t i=(uint32_t)li+1u;i<index;++i)if(g_doc.nodes[i].text_off&&node_descends_from_main(i,(uint32_t)li))return 0;int32_t list=g_doc.nodes[li].parent;if(list>0&&g_doc.nodes[list].tag==B_TAG_OL){uint32_t ordinal=0u;for(int32_t c=g_doc.nodes[list].first_child;c>=0;c=g_doc.nodes[c].next_sibling)if(g_doc.nodes[c].tag==B_TAG_LI){++ordinal;if(c==li)break;}if(ordinal>99u)ordinal=99u;char pfx[6];uint32_t w=0u;if(ordinal>=10u)pfx[w++]=(char)('0'+ordinal/10u);pfx[w++]=(char)('0'+ordinal%10u);pfx[w++]='.';pfx[w++]=' ';pfx[w]=0;b_copy(out,cap,pfx);}else b_copy(out,cap,"- ");return 1;
}
...
static void paint_document(void)
{
    ...
    for(uint32_t pass=0u;pass<2u&&used<g_paint_count;++pass) for(uint32_t i=1u;i<g_doc.node_count&&used<g_paint_count;++i) {
        ...
        if(n->text_off){uint32_t ignored=0u;uint32_t inner=n->w>n->style.padding_left+n->style.padding_right?n->w-n->style.padding_left-n->style.padding_right:n->w;const char*source=browser_document_string(&g_doc,n->text_off);char transformed[256],listed[256],prefix[8];if(n->style.text_transform){uint32_t w=0u;int word=1;while(source[w]&&w+1u<sizeof(transformed)){char c=source[w];if(n->style.text_transform==1u&&c>='a'&&c<='z')c=(char)(c-32);else if(n->style.text_transform==2u&&c>='A'&&c<='Z')c=(char)(c+32);else if(n->style.text_transform==3u&&word&&c>='a'&&c<='z')c=(char)(c-32);transformed[w++]=c;word=c==' '||c=='\n'||c=='\t';}transformed[w]=0;source=transformed;}if(list_item_prefix(i,prefix,sizeof(prefix))){b_copy(listed,sizeof(listed),prefix);append_text(listed,sizeof(listed),source);source=listed;}(void)browser_text_wrap(&n->style,source,n->style.nowrap?0x100000u:inner,p->text,sizeof(p->text),&ignored);}else p->text[0]=0;
        ...
        (void)g_api->gfx_obj_set_z(p->object,4+n->style.z_index);(void)g_api->gfx_obj_set_outline_width(p->object,0u);(void)g_api->gfx_obj_set_outline_alpha(p->object,0u);
        ...
        if(p->background_object){
            ...
            (void)g_api->gfx_obj_set_z(p->background_object,2+n->style.z_index);
...
static int update(const sacx_api *api)
{
    ...
    else if(g_load_state==LOAD_SCRIPTS)pump_external_scripts();
    ...
    if(api->mouse_get_state(&mouse)==0&&api->window_focused(g_window)){
        ...
        if(ly>=UI_DOCUMENT_Y){
            ...
            else if(g_load_state==LOAD_DOCUMENT||g_load_state==LOAD_STYLES||g_load_state==LOAD_SCRIPTS)cursor=SACX_MOUSE_CURSOR_WAIT;

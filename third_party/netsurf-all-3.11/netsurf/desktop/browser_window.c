			      ...
			      struct content_rfc5988_link *link)
{
	...
	/* Dihscover has no image decoder yet, so a favicon fetch only creates
	 * unsupported work while the document is loading. */
	(void)c;
	(void)bw;
	(void)link;
	return NSERROR_OK;
...
#ifdef DIHSCOVER_FREESTANDING
			      /* DihOS already parsed and supplies Content-Type.  Do not
			       * defer text/html creation for feed sniffing: the backend
			       * delivers the buffered response through a cooperative poll. */
			      fetch_flags,
#else

...
#define DIHSCOVER_FACTORY_LOG(text) dihscover_netsurf_log(text)
#else
#define DIHSCOVER_FACTORY_LOG(text) do {} while (0)
#endif
		...
		lwc_string *effective_type)
{
	...
	if (handler == NULL) {
		DIHSCOVER_FACTORY_LOG("Dihscover NetSurf core: no handler for MIME");
		return NULL;
	}
	...
	if (error != NSERROR_OK) {
		DIHSCOVER_FACTORY_LOG("Dihscover NetSurf core: handler create failed");
		return NULL;
	}

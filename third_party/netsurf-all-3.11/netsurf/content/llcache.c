...
#define DIHSCOVER_LLCACHE_LOG(text) dihscover_netsurf_log(text)
#else
#define DIHSCOVER_LLCACHE_LOG(text) do {} while (0)
#endif
...
llcache_fetch_parse_cache_control(llcache_object *object, char *value)
{
	...
	if (error != NSERROR_OK) {
#ifdef DIHSCOVER_FREESTANDING
		DIHSCOVER_LLCACHE_LOG("Dihscover NetSurf llcache: event handler returned an error; aborting fetch");
#endif
		/* Ignore parse errors */
		return NSERROR_OK;
	}
...
static void llcache_fetch_callback(const fetch_msg *msg, void *p)
{
	...
	switch (msg->type) {
	case FETCH_HEADER:
		DIHSCOVER_LLCACHE_LOG("Dihscover NetSurf llcache: received header");
	...
	case FETCH_DATA:
		DIHSCOVER_LLCACHE_LOG("Dihscover NetSurf llcache: received page data");
		/* Received some data */
	...
	case FETCH_FINISHED:
		DIHSCOVER_LLCACHE_LOG("Dihscover NetSurf llcache: received finished");
		/* Finished fetching */
	{
		uint8_t *temp;
	 ...
	 * This must remain deferred: callbacks can add/remove cache users, and
	 * re-entering the traversal here can leave the user list cyclic. */

...
extern void dihscover_netsurf_log(const char *text);
#define DIHSCOVER_LOG(text) dihscover_netsurf_log(text)
#else
#define DIHSCOVER_LOG(text) do {} while (0)
#endif
		...
		lwc_string *effective_type)
{
	...
	if (entry == NULL) {
		...
		entry->content = content_factory_create_content(ctx->llcache,
				...
				effective_type);
		if (entry->content == NULL) {
			DIHSCOVER_LOG("Dihscover NetSurf core: content creation returned NULL");
			free(entry);
			return NSERROR_NOMEM;
		}
		DIHSCOVER_LOG("Dihscover NetSurf core: content object created");
		...
		lwc_string *effective_type)
{
	...
	if ((effective_type != NULL) &&
	    hlcache_type_is_acceptable(effective_type,
				       ctx->accepted_types,
				       &type)) {
		DIHSCOVER_LOG("Dihscover NetSurf core: accepted MIME, creating content");
		...
		DIHSCOVER_LOG("Dihscover NetSurf core: rejected MIME as unacceptable");
			 ...
			 void *pw)
{
	...
	case LLCACHE_EVENT_HAD_HEADERS:
		DIHSCOVER_LOG("Dihscover NetSurf hlcache: processing headers");
		...
		if (error == NSERROR_OK || error == NSERROR_NOT_FOUND) {
			...
			if (error != NSERROR_OK && error != NSERROR_NEED_DATA)
				DIHSCOVER_LOG("Dihscover NetSurf hlcache: header migration failed");
	...
	case LLCACHE_EVENT_HAD_DATA:
		DIHSCOVER_LOG("Dihscover NetSurf hlcache: processing page data");
		...
		error = hlcache_migrate_ctx(ctx, effective_type);
		if (error != NSERROR_OK && error != NSERROR_NEED_DATA)
			DIHSCOVER_LOG("Dihscover NetSurf hlcache: data migration failed");
